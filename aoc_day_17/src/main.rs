...
fn main() {
    ...
    let mut board = Board::new(8 + 20, 8 + 20, 20);

    let offset = Point::new(10, 10, 10);
    for (y, line) in input.lines().enumerate() {
        for (x, c) in line.chars().enumerate() {
            if c == '#' {
                board.set(offset + Point::new(x as isize, y as isize, 0));
            }
        }
    }
    ...
    println!("Part 1: {}", board.calc_alive());
}
...
impl Display for Board {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        for (z, field) in self.cells.iter().enumerate() {
            writeln!(f, "Field: ({})", z)?;
...
impl Board {
    fn new(x: usize, y: usize, z: usize) -> Board {
        Board {
            cells: vec![vec![vec![false; x]; y]; z],
        }
    }
    ...
    fn set(&mut self, point: Point) {
        self.cells[point.z as usize][point.y as usize][point.x as usize] = true;
    }
        ...
        for z in -1 as isize..=1 {
            for y in -1 as isize..=1 {
                for x in -1 as isize..=1 {
                    let offset = Point::new(x, y, z);
                    if !offset.is_origin()
                        && self.get(offset + *location).unwrap_or(false)
                    {
                        count += 1;
                    }
                }
                    ...
                    .map(|(y, line)| {
                        line.iter()
                            ...
                            .map(|(x, value)| {
                                self.update_cel(
                                    *value,
                                    Point::new(x as isize, y as isize, z as isize),
                                )
                            })
                            .collect()
                    })
